...
% length = 10000;
% ratios = linspace(0.8,1.0,2);
ratios = 0.9;
streams = 5:7;
A = zeros(size(length,2),15);
S = struct;
results = struct;
...
for i = 1:size(ratios,2)
    ...
    for s = 1:size(streams,2)
        ...
        for l = 1:size(length,2)
            ...
            toc;                
            A(l,:) = [result.Power_total_sold_avg/1e3,...
        ...
        resultss(s).results = results;
    end
    S(i).results = resultss;
end
save('S_ratios_vs_n_streams_vs_res_length.mat', 'S');